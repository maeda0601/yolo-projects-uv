...
class CameraDetectionApp:
    ...
    def load_model(self):
        ...
        try:
            ...
            # モデルのクラス情報を取得（data.yamlのクラス名を優先）
            if hasattr(self.model, 'names') and self.model.names:
                model_names = self.model.names
                yaml_classes = self.load_classes()
                if len(yaml_classes) == len(model_names) and yaml_classes != ["object"]:
                    # data.yamlのクラス名を優先して使用
                    self.classes = yaml_classes
                else:
                    self.classes = list(model_names.values())
